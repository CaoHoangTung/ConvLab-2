...
if __name__ == '__main__':
    ...
    model = JointENVIBERT(config['model'], DEVICE, dataloader.tag_dim, dataloader.intent_dim, dataloader.intent_weight)
    model.to(DEVICE)
    ...
    for experiment_id in range(experiment_count):
        writer = SummaryWriter(f"{log_dir}-{experiment_id}")
        writer.add_text(f'config-{experiment_id}', json.dumps(config))
        ...
        for step in range(1, max_step + 1):
            ...
            if step % check_step == 0:
                ...
                if F1 > best_val_f1:
                    best_val_f1 = F1
                    torch.save(model.state_dict(), os.path.join(output_dir, f'pytorch_model-{experiment_id}.bin'))
        ...
        model_path = os.path.join(output_dir, f'pytorch_model-{experiment_id}.bin')
        zip_path = config['zipped_model_path']
        print('zip model to', zip_path)

        with zipfile.ZipFile(f"{zip_path}.{experiment_id}", 'w', zipfile.ZIP_DEFLATED) as zf:
            zf.write(model_path)
