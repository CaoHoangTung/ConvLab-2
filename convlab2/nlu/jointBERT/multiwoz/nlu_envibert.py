...
class BERTNLU(NLU):
    ...
    def predict(self, utterance, context=list()):
        ...
        ori_word_seq = nltk.word_tokenize(utterance)
        print("WORDS", ori_word_seq)
        ...
        if self.use_context:
            ...
            context_seq = self.dataloader.tokenizer.encode('<s> ' + ' </s> '.join(context[-3:]))
            context_seq = context_seq[:512]
        else:
            context_seq = self.dataloader.tokenizer.encode('</s>')
        intents = []
        da = {}
