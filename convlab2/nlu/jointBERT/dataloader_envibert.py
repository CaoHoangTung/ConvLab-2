...
import torch
import random
from .utils.XLMRobertaTokenizer import XLMRobertaTokenizer
import math
...
class Dataloader:
    def __init__(self, intent_vocab, tag_vocab, pretrained_weights):
        ...
        self.tokenizer = XLMRobertaTokenizer(pretrained_weights)
        self.data = {}
    ...
    def load_data(self, data, data_key, cut_sen_len, use_bert_tokenizer=True):
        ...
        for d in self.data[data_key]:
            ...
            d[4] = self.tokenizer.encode('<s> ' + ' </s> '.join(d[4]))
    ...
    def bert_tokenize(self, word_seq, tag_seq):
        ...
        basic_tokens = nltk.word_tokenize(' '.join(word_seq))
        accum = ''
        i, j = 0, 0
        for i, token in enumerate(basic_tokens):
            ...
            for sub_token in self.tokenizer.tokenize(basic_tokens[i]):
